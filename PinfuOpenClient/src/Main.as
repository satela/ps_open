...
//			return;
			...
			Userdata.instance.curRandomStr = HttpRequestUtil.resPrefix + "version.json?" + (new Date()).getTime().toString();
...
//				Laya.stage.scaleX = screenWidth/1920;
		...
		public function startInit():void
		{
			...
			//激活资源版本控制，版本文件由发布功能生成
			ResourceVersion.enable(HttpRequestUtil.resPrefix + "version.json?" + (new Date()).getTime().toString(), Handler.create(this, this.onVersionLoaded), ResourceVersion.FILENAME_VERSION);
			Laya.stage.on(Event.RESIZE,this,onResizeBrower);
		...
		private function onVersionLoaded(e:Event):void {
			...
			var prefix:String = HttpRequestUtil.resPrefix;

			Laya.loader.load([{url:prefix + "res/atlas/comp.atlas",type:Loader.ATLAS},{url:prefix + "res/atlas/commers.atlas?" + Userdata.instance.version,type:Loader.ATLAS},
				
				{url:prefix + "res/atlas/order1.atlas?" + Userdata.instance.version,type:Loader.ATLAS},
				{url:prefix + "res/atlas/upload1.atlas?" + Userdata.instance.version,type:Loader.ATLAS},
				{url:prefix + "res/atlas/usercenter1.atlas?" + Userdata.instance.version,type:Loader.ATLAS},
				{url:prefix + "res/atlas/mainpage.atlas?" + Userdata.instance.version,type:Loader.ATLAS},
				{url:prefix + "res/atlas/chargeact.atlas?" + Userdata.instance.version,type:Loader.ATLAS},
				{url:prefix + "res/atlas/uiCommonUpdate.atlas?" +  Userdata.instance.version,type:Loader.ATLAS},
				{url:prefix + "res/atlas/iconsNew.atlas?" + Userdata.instance.version,type:Loader.ATLAS},
		...
		private function onConfigLoaded():void {
			...
			var urlarr:Array = Browser.window.location.href.split("/");
			
			
			var platUrl:String = urlarr[3];
			
			
			if(HttpRequestUtil.isLocal)
				HttpRequestUtil.httpUrl = "https://www.yin.vip/api/" + "open/";
			else
				HttpRequestUtil.httpUrl = "https://www.yin.vip/api/" + platUrl + "/";
