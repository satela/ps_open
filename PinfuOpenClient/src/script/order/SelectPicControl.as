		...
		private function seletPicToOrder(data:Array):void
		{
			...
			if(param is MaterialItemVo)
			{
				...
				else
				{
					...
					if((param.preProc_Code == OrderConstant.UNNORMAL_CUT_TECHNO || param.preProc_Code == OrderConstant.UNNORMAL_CUT_TECHNO_UV) && this.checkIsValidYixing(data[1]))
					{
						texture = data[1];
					}
					else if((param.preProc_Code == OrderConstant.UNNORMAL_CUT_TECHNO || param.preProc_Code == OrderConstant.UNNORMAL_CUT_TECHNO_UV) && !this.checkIsValidYixing(data[1]))
					{
					...
					if(param.preProc_Code == OrderConstant.UNNORMAL_CUT_TECHNO || param.preProc_Code == OrderConstant.UNNORMAL_CUT_TECHNO_UV)
